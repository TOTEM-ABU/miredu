...
import { 
  ...
  ChevronRight, Save, Loader2, AlertCircle, CalendarDays 
} from 'lucide-react';
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '2.25rem', fontWeight: 900, marginBottom: '0.4rem' }}>Davomat 📅</h1>
        <p style={{ color: '#475569', fontWeight: 600 }}>O'quvchilarning darsga qatnashishini nazorat qiling</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '2rem' }}>
        ...
        <div className="content-card">
          ...
          <div className="field-wrap">
            ...
            >
              <option value="" style={{ background: '#0f172a' }}>Guruhni tanlang...</option>
              {groups.map(g => (
                <option key={g.id} value={g.id} style={{ background: '#0f172a' }}>{g.name}</option>
              ))}
            </select>
          </div>
          {selectedGroup && (
            <div style={{ marginTop: '1rem', display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
              {selectedGroup.days?.split(',').map(d => (
                <span key={d} style={{ 
                  fontSize: '0.7rem', fontWeight: 800, padding: '0.3rem 0.6rem', 
                  borderRadius: '0.5rem', background: 'rgba(99,102,241,0.1)', color: 'var(--primary)'
                }}>
                  {DAYS_MAP[d]}
                </span>
              ))}
        ...
        <div className="content-card">
          ...
          {!isDateValid() && selectedGroup && (
            <div style={{ marginTop: '0.8rem', display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#ef4444' }}>
              <AlertCircle size={14} />
              <span style={{ fontSize: '0.75rem', fontWeight: 700 }}>Ushbu kunda guruhda dars yo'q!</span>
            </div>
          )}
        </div>
      </div>
      ...
      {/* Student List */}
      <div className="content-card" style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: 0, overflow: 'hidden' }}>
        <div style={{ padding: '1.5rem', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h3 style={{ fontWeight: 850, fontSize: '1.1rem' }}>Talabalar Ro'yxati</h3>
          {selectedGroup && (
            <button 
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '1.5rem' }}>
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '4rem 0', color: '#475569' }}>
              ...
              <p style={{ fontWeight: 700 }}>Davomat qilish uchun guruhni tanlang</p>
            </div>
          ) : students.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '4rem 0', color: '#475569' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {students.map((student, idx) => (
                ...
                >
                  <div style={{ width: 40, height: 40, borderRadius: '50%', overflow: 'hidden', border: '2px solid rgba(99,102,241,0.2)', flexShrink: 0 }}>
                    <img 
                      src={student.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(student.firstName)}&background=6366f1&color=fff`} 
                      style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                      alt={student.firstName}
                    />
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 800, fontSize: '0.95rem', color: '#f1f5f9' }}>{student.firstName} {student.lastName}</div>
                    <div style={{ fontSize: '0.75rem', color: '#64748b', fontWeight: 600 }}>ID: {student.id.slice(0, 8)}...</div>
                  </div>

                  {/* Status Switcher */}
                  <div style={{ display: 'flex', background: 'rgba(0,0,0,0.2)', padding: '0.25rem', borderRadius: '0.75rem', gap: '0.25rem' }}>
                    {[
                      { id: 'PRESENT', label: 'Keldi', icon: CheckCircle2, color: '#10b981' },
                      { id: 'ABSENT', label: 'Yo\'q', icon: XCircle, color: '#ef4444' },
                      { id: 'LATE', label: 'Kech', icon: Clock, color: '#f59e0b' }
                    ].map(st => {
                      ...
                      return (
                        <button
                          ...
                          style={{
                            ...
                            padding: '0.5rem 0.8rem', borderRadius: '0.6rem', border: 'none',
                        ...
                        >
                          <st.icon size={14} />
                          <span style={{ display: isActive ? 'inline' : 'none' }}>{st.label}</span>
                        </button>
                      );
                    })}
                  </div>
        ...
        {selectedGroup && (
          <div style={{ padding: '1.5rem', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'flex-end' }}>
            ...
            >
              {saving ? <Loader2 className="spin" size={20} /> : <><Save size={18} /> Saqlash</>}
            </button>
          </div>
        )}
      </div>
